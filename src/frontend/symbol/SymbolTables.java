...
public class SymbolTables {     // 整个程序的符号表
    private ArrayList<HashMap<String, Symbol>> symbolTables = new ArrayList<>();
    ...
    public Symbol findSymbol(String name) {
        for (int i = symbolTables.size() - 1; i >= 0; i--) {
            ...
            if (curSymbolTable.containsKey(name)) {
                return curSymbolTable.get(name);    // 已声明的标识符
            }
        }
        return null;   // 未声明的标识符
    }

    // 匹配函数的形参与实参 检测错误d【函数参数个数不匹配】，e【函数参数类型不匹配】
    public ErrorType matchFuncParam(ArrayList<Param> formalParams, ArrayList<Param> realParams) {
        if (formalParams.size() != realParams.size()) {
            ...
            for (int i = 0; i < formalParams.size(); i++) {
                if (!formalParams.get(i).equals(realParams.get(i))) {
